...
public class EntityTestWeb extends AbstractPrjMgrWebTest {
  ...
  public void testEntityBadNames() throws Exception {
    ...
    testWebResponse(
        "Wrong result reading project name '" +
            TestPrjMgrBaseConstants.TEST_PRJ_NAME + "'",
        new WebResponse(404),
  ...
  public void testTestEntityGood() throws Exception {
    ...
    String[][][] dset = new String[][][] { FUTILS.getJsonTestSetFull(),
        FUTILS.getJsonDemoSetFull() };

    for (String[][] jset : dset)
      for (int i = 0; i < jset.length; i++)
        testWebResponse("Fail getting " + jset[i][0] + " file",
            new WebResponse(jset[i][1]), readGet(getEntityPath() + jset[i][0]));
  }
  ...
  public void testEntityCreateSimple() throws Exception {
    ...
    for (String fname : new String[] { "ds1", "dst" })
      testWebResponse("Create " + fname,
          new WebResponse(200, "@\\{\"request_id\":,\"error\":\\{\"id\":2," +
    ...
    testWebResponse("Renaming ds1" + " to itself",
        new WebResponse("@\\{\"request_id\":,\"error\":\\{\"id\":18," +
            ...
            "\"info\":\"Can not rename file \\\\\\\\\\\\\".*" + dsname +
            "/ds1" + "." + FUTILS.getBaseExt() +
            "\\\\\\\\\\\\\" to itself\"\\}\\}@"),
    ...
    testWebResponse(
        new WebResponse("@\\{\"request_id\":,\"error\":\\{\"id\":1," +
            ...
            "\"info\":\"Entry \\\\\\\\\\\\\".*" + dsname + "/ds2." +
            FUTILS.getBaseExt() + "\\\\\\\\\\\\\" not found\"\\}\\}@"),
  ...
  private void checkUploadDownload(String fname, String text, String json,
                      boolean overwrite) throws ClientProtocolException, IOException {
    InputStream in = new ByteArrayInputStream(text.getBytes());
    testWebResponse(new WebResponse(json),
       uploadFile(getEntityPath() + "upload/" + 
           fname + (overwrite ? "?overwrite=true" : ""), fname, in));
