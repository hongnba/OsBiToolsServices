...
public class GenericTest {
  ...
  public void testFileDir() throws WsSrvException {
    assertEquals("test", GenericUtils
        .getFileDir(TestConstants.SRC_DIR, "test.config").getName());
    assertEquals("src",
        GenericUtils.getFileDir(TestConstants.SRC_DIR, "test").getName());
  }
  ...
  public void testDirCopy() throws IOException, WsSrvException {
    ...
    for (int i = 1; i <= TEST_FNUM; i++)
      GenericUtils.copyToFile("test" + i,
          src.getAbsoluteFile() + File.separator + "test" + i);
    ...
    for (int i = 1; i <= TEST_FNUM; i++)
      assertTrue("Destination file test" + i + " not found",
          new File(dst.getAbsoluteFile() + File.separator + "test" + i)
              .exists());

    // Create sub-directory in source folder
    String sdir = src.getAbsolutePath() + File.separator + "test";

    assertTrue("Error creating sub-directory in source folder",
        new File(sdir).mkdir());
    ...
    for (int i = TEST_FNUM + 1; i <= TEST_FNUM * 2; i++)
      GenericUtils.copyToFile("test" + i, sdir + File.separator + "test" + i);
    ...
    for (int i = TEST_FNUM + 1; i <= TEST_FNUM * 2; i++)
      assertTrue("Destination file test" + i + " not found",
          new File(sdest.getAbsolutePath() + File.separator + "test" + i)
              .exists());

    // Clean up
    GenericUtils.delDirRecurse(src, true);
    GenericUtils.delDirRecurse(dst, true);
  }

  private void doCheckDeleteDir(File dir) throws IOException, WsSrvException {
    if (dir.exists()) {
      if (dir.isDirectory())
        GenericUtils.delDirRecurse(dir, true);
      else
        GenericUtils.deleteFile(dir);
    }
