...
public class BaseUtils {
  ...
  public static String getFileExt(String name) throws IOException {
    ...
    if (pos == -1)
      throw new IOException("Extension not found in name \\\"" + name + "\\\"");
  ...
  public static String checkFileExt(String name, String ext)
    ...
    if (!fext.equals(ext))
      throw new IOException("Expected " + ext + " extension but found \\\"" + fext + "\\\"");
   ...
   */
  public static String checkFileExt(String name, String ext,
      HashSet<String> extl) throws IOException {
    int len = name.length();
    ...
    if (!extl.contains(ext)) {
      ...
      throw new IOException("Expected " + (extl.size() > 1 
          ? "one of " + sl.substring(1) 
          : ext) + " extension" + (extl.size() > 1 ? "s" : "") + " but found \\\"" +
              ext + "\\\"");
    }
  ...
  public static void renameFile(String base, File oldName, File newName,
      String sdir) throws IOException {
    if (!oldName.renameTo(newName))
      throw new IOException("Can not rename " +
